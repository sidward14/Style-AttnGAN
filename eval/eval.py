...
from model import TRANSFORMER_ENCODER, RNN_ENCODER, G_NET, G_NET_STYLED
...
def generate(caption, wordtoix, ixtoword, text_encoder, netG, blob_service, copies=2):
    ...
    #######################################################
    if isinstance( text_encoder, RNN_ENCODER ):
        hidden = text_encoder.init_hidden(batch_size)
        words_embs, sent_emb = text_encoder(captions, cap_lens, hidden)
    elif isinstance( text_encoder, GPT2Model ):
        words_embs = text_encoder( captions )[0].transpose(1, 2)
